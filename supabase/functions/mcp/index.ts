  ...
  server.registerTool(
    "get_sam_measure_stats",
    {
      ...
      description:
        ...
        "⚠️ TWO HIT RATES, NEVER INTERCHANGEABLE. `hit_rate_all` counts every loop iteration. `hit_rate_attempted` counts only iterations he actually played in, excluding `sat_out_iterations` — cycles where EVERY beat of the measure was a miss with nothing struck at all, which is what a loop left running while he resets his hands records. Both are true; the second is the one that answers \"which measures do I miss\". A measure can read 40% all / 95% attempted purely because the loop ran on without him, so ALWAYS quote sat_out_iterations beside hit_rate_all, and rank difficulty on hit_rate_attempted (weakest_measures already does). " +
        "WRONG NOTES come from `extra` rows (a keystroke that matched no beat) and from the pitches carried on `miss` rows (a wrong attempt AT a beat), counting ONLY pitches the beat did NOT expect — a failing chord records every key struck, the correct ones included, and those are the part he got right, not mistakes. Each pitch is counted ONCE PER PASS per measure, so hammering one wrong key is one occurrence, and one fumble can never be counted twice. A pitch is listed only when it recurs in at least 3 distinct passes — below that it is noise. " +
        "⚠️ KNOWN GAP IN WRONG NOTES: a note TIED OVER into a measure is sounding but never struck, so it is absent from that beat's expected_notes. Playing a snippet that starts mid-phrase, he strikes those notes to place his hand and they are logged as recurring wrong notes though they are not errors. Treat recurring wrong notes IN THE FIRST BAR OF A SNIPPET, or in any bar entered from a rest, with suspicion, and check them against the score before reporting them as mistakes. " +
