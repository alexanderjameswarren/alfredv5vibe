  ...
  server.registerTool(
    "get_job_application_sources",
    {
      ...
      description:
        ...
        "COUNTS APPLICATIONS ONLY: rows with status 'considering' or 'passed' were never submitted and are excluded from EVERY count here, `total` included — counting them would put roles Alex merely looked at into the denominator and lower every source's response rate. `not_applied_excluded` reports how many were left out. " +
  ...
  server.registerTool(
    "get_recent_clips",
    {
      ...
      description:
        ...
        "Returns per clip: id, source, url, title, captured_at, inbox_id, slice_count, screenshot_note, page_width and page_height (the ORIGINAL page size in CSS pixels, before the extension scaled it to 1280 wide — so you never have to estimate how tall a page was), page_text and links, plus flags saying whether the stored text or screenshot is incomplete. NO IMAGES — get_clip_slices does that. " +
        ...
        "screenshot_truncated true means you are NOT looking at the whole page. DO NOT GUESS WHY — screenshot_note says, in words, what the screenshot is and what went wrong if anything. Quote it rather than inventing a cause. A common one is a page whose content scrolls inside its own box, which no amount of capturing fixes; the TEXT is complete either way, so answer from page_text and say the screenshot is partial. " +
