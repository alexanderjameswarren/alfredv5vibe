  ...
  server.registerTool(
    "get_recent_clips",
    {
      ...
      description:
        ...
        "TWO KINDS OF SCREENSHOT, and capture_mode says which. 'visible' is the everyday one: a single photograph of what was ON SCREEN when Alex clipped, roughly one viewport, DELIBERATELY not the whole page and NOT a failure — screenshot_truncated is false for it because nothing went wrong. Never describe a visible clip as the full page, and never describe it as truncated or incomplete; say it shows the visible screen. 'full' means the whole page was photographed, which Alex triggers separately. " + "screenshot_truncated true means a FULL capture did not get the whole page. DO NOT GUESS WHY — screenshot_note says, in words, what the screenshot is and what went wrong if anything. Quote it rather than inventing a cause. A common one is a page whose content scrolls inside its own box, which no amount of capturing fixes. THE TEXT IS COMPLETE IN EVERY MODE, so answer from page_text and describe the picture for what it is. " +
