...
## Status: M1 through M4 part 1 verified. M5, session metadata, pass playback speed and pass accuracy implemented, awaiting verification.
...
- [x] `hits`, `misses`, `notes_played`, `hand_mode` on `sam_passes`, taken from
      the per-playthrough counters `usePracticeSession` already keeps
- [x] `accuracy_percent` generated by Postgres from those, so there is no second
      definition of accuracy. Partials excluded, matching `accuracyOf`
- [x] NULL vs 0 kept apart: NULL means unmeasurable (nothing arrived, or the row
      predates the column), 0 means measured and every note wrong
- [x] Not backfilled — same convention as `playback_speed`
- [x] `get_sam_passes` returns all of it, plus `exclude_zero_note` and
      `only_zero_note`
- [x] Pass credit moved ahead of `setLoopIteration` so the counters read belong
      to the playthrough that just finished. Detection rules unchanged
- [x] `check_platform_conformance` → CONFORMANT (pre-migration baseline)

### Fingering mode button moved to the snippet row
- [x] Fingering mode, Diff and Show Imported now sit at the far end of the
      Snippet toggle row — the last row before the score, previously holding
      only a small toggle
- [x] No row added: both sides of that row are already `min-h-[44px]`
- [x] The transport row gets its width back
- [x] Fingering bar and Undo unchanged, still appearing only when the mode is on

### Pass playback speed (item D, built)
Needs a migration — unlike the session metadata, these are real columns, because
`effective_bpm` has to be generated by Postgres to be drift-proof.
`docs/migrations/2026-09-16-sam-passes-playback-speed.sql`.

- [x] `playback_speed integer` on `sam_passes`, written at credit time from the
      speed in force at the finish line
- [x] `effective_bpm integer generated always as
      ((round((bpm * playback_speed) / 100.0))::integer) stored`
- [x] NOT backfilled. NULL means "not recorded"; 100 means "recorded as full
      speed". `effective_bpm` is NULL wherever the speed is
- [x] Column comments state the before/after line explicitly, and `bpm`'s own
      comment now points at `effective_bpm`
- [x] Index on `(song_id, effective_bpm)`
- [x] `get_sam_passes` returns both facts and the derived value; description
      spells out that NULL speed is not 100
- [x] `check_platform_conformance` → CONFORMANT (pre-migration baseline;
...
The brief said a pass with no keyboard records zero hits and zero misses. It
does not, and the difference decided the design.

`ScrollEngine` raises a miss on elapsed time alone — the
`elapsed > evt.targetTimeMs + timingWindowMs` branch never consults MIDI — so a
playthrough with no keyboard attached scores a miss on EVERY beat: 0 hits, N
misses. That is byte-identical to playing every note wrong, and is the same
phenomenon reported at session level for session 84052716 (0 hits, 6 misses with
no keyboard), one level up.

So hits and misses alone cannot separate "unmeasured" from "measured and bad",
and a fourth column was needed: `notes_played`, a count of MIDI notes that
actually arrived. The miss path passes `played: []` and only the match path
carries notes, so it stays at 0 for a playthrough nobody played. That is the
honest discriminator, and it is what makes `accuracy_percent` NULL rather than
0% for a playback test — exactly the distinction the brief asked for, reached by
a different route than it assumed.

`accuracy_percent` is generated rather than written, for the same reason as
`effective_bpm`: derived from stored facts by Postgres, so it cannot drift and
cannot be written by mistake. Partials sit outside the ratio, matching
`accuracyOf` in `usePracticeSession`, so a pass and its session agree on what
accuracy means.

**One ordering change was required.** `creditPass()` now runs before
`setLoopIteration(n)` in `handleLoopCount`. `setLoopIteration` rotates the
per-playthrough counters, and the pass needs the ones belonging to the
playthrough that just finished. Reading `lastPlaythroughRef` instead would have
been subtly wrong: that ref is only updated when the outgoing playthrough had
scored beats, so a zero-note playthrough would have silently inherited the
PREVIOUS one's hits and misses — precisely the test data this exists to make
identifiable. Neither call depends on the other, so detection is unchanged: the
same signal credits the same passes at the same instant.

**Not added:** `partials`. The accuracy ratio excludes them by definition, so
nothing here needs them. Worth adding later if partials become interesting in
their own right.

#### Pass playback speed — why both facts and a generated third

Storing only the multiplier keeps both facts but makes every reader responsible
for combining them, and a reader that forgets gets a plausible wrong number
rather than an error. Storing only an effective tempo is honest about what was
heard but destroys the difference between "practised at 48" and "practised at 60
with the track slowed" — different practice decisions that should not collapse
into one number.

So both, plus `effective_bpm` as a STORED generated column. Postgres computes it
from the two facts, so it cannot drift; readers who want the simple answer get
it without arithmetic; nothing is lost.

**The before/after line is real and cannot be closed.** Every pass written
before 2026-09-16 has a bpm and no speed, and the speed is unrecoverable — it
was never captured anywhere. `sam_sessions.settings.playbackSpeed` records a
start-of-session snapshot, and joining through it would be inventing data in
exactly the way the orphaned-session analysis ruled out. So `playback_speed`
stays NULL for those rows, `effective_bpm` NULL with it, and a reader can always
tell which side of the line a row falls on. Backfilling 100 would have erased
that distinction and made every historical pass look like a recorded fact.

**Scope.** Speed only applies to audio-backed songs — SAM shows Playback Speed %
only when an MP3 is attached — so most passes will record 100. That does not
make the column pointless: the passes it does affect are silently wrong today,
and silently wrong is worse than missing.
