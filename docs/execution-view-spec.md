...
async function startNowFromItem(itemId) {
  ...
  // 2. Create Event (scheduled for today)
  const event = {
    id: uid(),
    intentId: intent.id,
    time: "today",
  ...
  // 3. Create Execution with frozen elements
  const itemElements = (item.elements || item.components || []).map(el => {
    const element = typeof el === "string"
      ? { name: el, displayType: "step", quantity: "", description: "" }
      : { ...el };
    return {
...
**Changes to ItemDetailView:**

Add a "Start" button in the header area, next to the "Edit Item" button:
...
**Guard:** If there's already an active execution, either prevent starting a new one (show a message) or close/cancel the existing one first. Recommend: prevent and show a notice — "You have an active execution. Complete or cancel it first."

**Testing for Block 5:**
- Open an Item with elements → click "Start Now" → lands in ExecutionDetailView with elements shown
- Verify Supabase has new intent (isIntention: true, itemId set), new event (time: "today"), and execution:active with frozen elements
- Open an Item with no elements → click "Start Now" → ExecutionDetailView shows title and notes only
- Complete the execution → intent gets archived (one-time), event gets archived
- The Item's Related Intentions section now shows the newly created intention
- Try to start a new execution while one is active → should be prevented
