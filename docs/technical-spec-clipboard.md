...
- A one-tap process button beside the trash can, enabled whenever the item is enriched (not only when it has a context, since memory-bound items have none). Consider tinting the preview when confidence is low.
